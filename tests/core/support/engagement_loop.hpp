...
// REQ-SAF-007, REQ-TRK-011, REQ-TRK-012 — the application loop the scenario
// tests drive.
//
...
class EngagementLoop {
 ...
 private:
  ...
  void fire(const pigeon::core::TargetTransition& transition, FrameOutcome& result) {
    const std::optional<pigeon::core::ServoAngles> angles = angles_for(*transition.aim_at);
    const pigeon::core::LinkStatus link_status = link_->is_available()
                                                     ? pigeon::core::LinkStatus::OK
    ...
    result.fire_status = link_->send_fire_command(command);
